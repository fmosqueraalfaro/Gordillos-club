...
export function AddExperienceSheet({
  ...
  const canSave = name.trim().length > 0 && rating > 0 && !busy

  async function handleSubmit(e: FormEvent) {
    e.preventDefault()
    if (!user) return
    if (!name.trim()) return setError("Ponele un nombre al lugar.")
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-display text-xl font-semibold text-ink">Nueva experiencia</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Cerrar"
